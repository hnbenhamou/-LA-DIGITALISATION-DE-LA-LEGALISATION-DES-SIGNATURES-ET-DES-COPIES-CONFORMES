...
class ListeDemandeEgal extends Component {
  constructor(props) {
   ...
   fetchData = async () => {
    try {
      const response = await fetch('http://localhost:8088/copie/pending', {
        method: 'GET',
      });
  ...
  handleRejection = async (id) => {
   try {
      const response = await fetch(`http://localhost:8088/copie/delete/${id}`, {
        method: 'DELETE',
      });
  ...
  handleValidation = async (id) => {
    try {
      const response = await fetch(`http://localhost:8088/copie/egalise/${id}`, {
        method: 'POST',
      });
      ...
      console.error('Erreurs de fetch :', error);
      alert("Demante égalisé");
    }
  }
                ...
                <Table className="table">
                  ...
                  <tbody>
                    {demandeEgal.map((demandeEgal) => (
                      <tr key={demandeEgal.id_c}>
                        <td>{demandeEgal.citoyen.cni}</td>
                        <td>{demandeEgal.date}</td>
                        
                        <td><a href={`http://localhost:8088/documents/demande_copie/${demandeEgal.document}`} target="_blank" rel="noopener noreferrer">
                        {demandeEgal.document}
                        </a>
                        </td>
                        
                        
                        <td><button onClick={() => this.handleValidation(demandeEgal.id_c)}>Egaliser</button></td>
                        <td><button onClick={() => this.handleRejection(demandeEgal.id_c)}>Rejeter</button></td>
                      </tr>
